...
async function verifyDeployment() {
  ...
  try {
    console.log('=== Navigating to temporary deployment ===');
    await page.goto('https://temporary-rapid-jasper-tnh29sb.vercel.app', { waitUntil: 'networkidle' });
    ...
    if (avatarCount2 > 0) {
      ...
      if (sair > 0) {
        ...
        const isHome = page.url().endsWith('/') || page.url().includes('temporary-rapid-jasper');
